...
class OsirisGridFileJAX(OsirisDataJAX):
    """
    ...
    def __init__(self, filename):
        ...
        data = jnp.array(self._file[variable_key][:])
        ...
        if len(axis) == 1:
            ...
            self._x = jnp.arange(self.grid[0], self.grid[1], self.dx)
        else: 
            grid = []
            for ax in axis: grid.append(self._file['AXIS/' + ax][()])
            self._grid = jnp.array(grid)
            self._nx = jnp.array(self._file[variable_key][()].transpose().shape)
            self._dx = (self.grid[:, 1] - self.grid[:, 0])/self.nx
            self._x = [jnp.arange(self.grid[i, 0], self.grid[i, 1], self.dx[i]) for i in range(self.dim)]
    ...
    def yeeToCellCorner(self, boundary=None):
        ...
        @partial(jax.jit, static_argnames=('field_name', 'boundary'))
        def _yee_1d_jitted(data, field_name, boundary):
            if field_name in ['b2', 'b3', 'e1']:
                if boundary == 'periodic':
                    return 0.5 * (jnp.roll(data, shift=1) + data)
                else:
                    return 0.5 * (data[1:] + data[:-1])
            elif field_name in ['b1', 'e2', 'e3']:
                if boundary == 'periodic':
                    return data
                else:
                    return data[1:]
        ...
        # @partial(jax.jit, static_argnames=('field_name', 'boundary'))
        def _yee_2d_jitted(data, field_name, boundary):
            '''
            ...
            if field_name in ['e1', 'b2']:
                if boundary == 'periodic': return 0.5 * (jnp.roll(data, shift=1, axis=0) + data)
                else: return 0.5 * (data[1:, 1:] + data[:-1, 1:])
            elif field_name in ['e2', 'b1']:
                if boundary == 'periodic': return 0.5 * (jnp.roll(data, shift=1, axis=1) + data)
                else: return 0.5 * (data[1:, 1:] + data[1:, :-1])
            elif field_name in ['b3']:
                if boundary == 'periodic': 
                    return 0.5 * (jnp.roll((0.5 * (jnp.roll(data, shift=1, axis=0) + data)), shift=1, axis=1) + (0.5 * (jnp.roll(data, shift=1, axis=0) + data)))
                else:
                    return 0.25 * (data[1:, 1:] + data[:-1, 1:] + data[1:, :-1] + data[:-1, :-1])
            elif field_name in ['e3']:
                if boundary == 'periodic': return data
                else: return data[1:, 1:]
            else:
                raise TypeError(f'This method expects magnetic or electric field grid data but received \'{field_name}\' instead')
        
        @partial(jax.jit, static_argnames=('field_name', 'boundary'))
        def _yee_3d_jitted(data, field_name, boundary):
            '''
            ...
            if field_name == 'b1':
                return 0.25 * (data[1:, 1:, 1:] + data[1:, :-1, 1:] + data[1:, 1:, :-1] + data[1:, :-1, :-1])
            elif field_name == 'b2':
                return 0.25 * (data[1:, 1:, 1:] + data[:-1, 1:, 1:] + data[1:, 1:, :-1] + data[:-1, 1:, :-1])
            elif field_name == 'b3':
                return 0.25 * (data[1:, 1:, 1:] + data[:-1, 1:, 1:] + data[1:, :-1, 1:] + data[:-1, :-1, 1:])
            elif field_name == 'e1':
                return 0.5 * (data[1:, 1:, 1:] + data[:-1, 1:, 1:])
            elif field_name == 'e2':
                return 0.5 * (data[1:, 1:, 1:] + data[1:, :-1, 1:])
            elif field_name == 'e3':
                return 0.5 * (data[1:, 1:, 1:] + data[1:, 1:, :-1])
        ...
        if self.dim == 1:
            if boundary == 'periodic' and self.name.lower() in ['b1', 'e2', 'e3']:
                raise ValueError("Periodic handling not applicable for this field.")
            self.data_centered = _yee_1d_jitted(self.data, self.name.lower(), boundary)
            return self.data_centered
        elif self.dim == 2:
            self.data_centered = _yee_2d_jitted(self.data, self.name.lower(), boundary)
            return self.data_centered
        elif self.dim == 3:
            self.data_centered =  _yee_3d_jitted(self.data, self.name.lower(), boundary)
            return self.data_centered
        else:
    ...
    def FFT(self, axis=(0, )):
        ...
        '''
        datafft = jnp.fft.fftn(self.data, axes=axis)
        self._FFTdata = jnp.fft.fftshift(jnp.fft.fftn(self.data, axes=axis), axes=axis)
...
class OsirisRawFileJAX(OsirisDataJAX):
    '''
    ...
    def __init__(self, filename):
        ...
        self.grid = jnp.array([self._file['SIMULATION'].attrs['XMIN'], self._file['SIMULATION'].attrs['XMAX']]).T
        ...
        for key in self._file.keys():
            ...
            self.data[key] = jnp.array(self._file[key][()])

            idx = jnp.where(self._file.attrs['QUANTS'] == str(key).encode('utf-8'))
            axis_data = {
...
class OsirisHISTJAX(OsirisDataJAX):
    ''''
