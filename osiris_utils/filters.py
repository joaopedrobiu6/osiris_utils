...
* the database creators (:mod:`osiris_utils.database.database`), which
  filter every raw frame while writing the tensors;
* the lazy diagnostic pipeline
  (:mod:`osiris_utils.postprocessing.filtering`,
  :class:`~osiris_utils.ar.anomalous_resistivity.AnomalousResistivity`),
  which applies the same filter per frame, on request, so an interactive
  session reproduces the tensors without building them.
