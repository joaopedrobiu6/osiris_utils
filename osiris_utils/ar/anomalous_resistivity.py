...
class TermFactory:
    ...
    def create_product_derivative_term(
        self, quantity1_type: str, quantity1_component: str, quantity2_type: str, quantity2_component: str, axis: str
    ...
    def create_velocity_field_term(self, velocity_type: str, velocity_component: str, field_type: str, field_component: str):
        ...
        return self.sim_mft[self.species][velocity_type][velocity_component] * self.sim_mft[field_type][field_component]

    def create_velocity_derivative_term(self):
        """Create velocity * velocity_derivative term."""
        return self.sim_mft[self.species]["vfl1"]["delta"] * self.sim_mft[self.species]["dvfl1_dx1"]["delta"]
...
class AnomalousResistivityABC(ABC):
    ...
    def compute_vlasov_electric_field(self):
        """Compute the Vlasov electric field."""
        pass

    @abstractmethod
    def compute_mean_field_terms(self):
        """Compute mean field terms for anomalous resistivity."""
        pass
...
class AnomalousResistivity(AnomalousResistivityABC):
    ...
    def __init__(self, simulation, species: str = "electrons", config: AnomalousResistivityConfig = None):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to initialize AnomalousResistivity: {e}") from None
    ...
    def _validate_species_inputs(self, simulation, species: str):
        ...
        if not hasattr(simulation, 'species') or species not in simulation.species:
            raise ValueError(f"Species '{species}' not found in simulation") from None
        ...
        for qty in required_quantities:
            try:
                ...
                raise ValueError(f"Required quantity '{qty}' not available for species '{species}'") from None
    ...
    def _validate_fields_inputs(self, simulation):
        ...
        for field in required_fields:
            try:
                ...
                raise ValueError(f"Required field '{field}' not available in simulation") from None
    ...
    def _compute_average_derivative(self):
        ...
        return Derivative_Diagnostic(self.sim_mft[self.species]["n"]["avg"] * self.sim_mft[self.species]["T11"]["avg"], "x1")
    ...
    def _compute_mft_terms(self, derivative_terms: dict[str, Derivative_Diagnostic]) -> dict[str, MFT_Diagnostic]:
        ...
        derivative_term_names = ["dnT11_dx_ad", "dnT11_dx_da", "dnT11_dx_dd", "dnT12_dx_ad", "dnT12_dx_da", "dnT12_dx_dd"]
    ...
    def _compute_eta_values(self, mft_terms: dict[str, MFT_Diagnostic]) -> dict[str, Any]:
        ...
        eta_dom_coefficients = {"term2": -1, "term4": 1, "term5": -1, "term6": -1, "term7": -1, "term8": -1, "term10": -1}

        # Compute eta using coefficients
        eta = sum(coeff * mft_terms[term]["avg"] for term, coeff in eta_coefficients.items() if term in mft_terms)

        eta_dom = sum(coeff * mft_terms[term]["avg"] for term, coeff in eta_dom_coefficients.items() if term in mft_terms)
...
def vlasov_electric_field(simulation, species: str = "electrons", config: AnomalousResistivityConfig = None):
    """
...
class AnomalousResistivity2(AnomalousResistivityABC):
    """
    Class to compute the anomalous resistivity from shock simulation data.
    ...
    def __init__(self, simulation, species: str = "electrons", config: AnomalousResistivityConfig = None):
        ...
        self.species = species
        self._config = config or AnomalousResistivityConfig(species=species)
        self._rqm = simulation[species]["n"]._species.rqm
        self._q = np.sign(self._rqm)
        self._m = np.abs(self._rqm)
        self._electron_mass = 1
        self._electron_charge = -1
    ...
    def _compute_vlasov_field_terms(self):
        ...
        if self._config.include_time_derivative:
            terms.append(self._rqm * self._simulation[self.species]["dvfl1_dt"])

        if self._config.include_convection:
            terms.append(self._rqm * self._simulation[self.species]["vfl1"] * self._simulation[self.species]["dvfl1_dx1"])

        if self._config.include_pressure:
            pressure_term = (self._q / self._simulation[self.species]["n"]) * (
                self._simulation[self.species]["dnT11_dx1"] + self._simulation[self.species]["dnT12_dx2"]
            )
            terms.append(pressure_term)
    ...
    def _compute_eta_values(self, mft_terms: dict[str, MFT_Diagnostic]) -> dict[str, Any]:
        ...
        eta_dom_coefficients = {"term2": self._q, "term4": 1, "term5": -1, "term6": -1, "term7": -1, "term8": -1, "term10": -1}
